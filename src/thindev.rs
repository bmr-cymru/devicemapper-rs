...
mod tests {
    ...
    fn test_udev_userspace(paths: &[&Path]) {
        ...
        fn validate(path_uuid: &Uuid, devnode: &Path) {
            ...
            println!("Looking for a path_uuid={}", path_uuid);
        ...
        fn set_new_fs_uuid(devnode: &Path) -> Uuid {
            ...
            .unwrap();
            umount2(tmp_dir.path(), MntFlags::empty()).unwrap();

            // Wait for udev events to complete: otherwise the event generated by xfs_admin
            // during xfs_set_uuid() is lost.
            udev_settle().unwrap();
